...
| FrameBus | 已实现 v2、4 槽、raw YUV/JPEG 格式、PTS/arrival/latest-frame | AHardwareBuffer 跨进程句柄仍是后续 backend |
| 本地 MP4 | MediaCodec → YUV_420_888 → NV21 → VCF2 | 不支持 DRM/secure 视频 |
| MJPEG | 有界解析；replacement 解码到 NV21，external 才输出 JPEG | 当前不支持 HLS/RTMP 客户端 |
| 前后替换协议 | adapter protocol v2、GET_INFO/PROBE/ATTACH/ACTIVATE/HEALTH、FD 附着、OEM metadata/secure/failure policy | 仓库没有任何目标 OTA 的 proprietary adapter |
| 精确 OTA 门禁 | system/vendor/provider/camera libs/graphics/adapter 哈希 | 每次 OTA 必须重新生成/验证 |
| BootGuard | 连续失败三次关闭 replacement，保留 external | 不能替代 bootloader/recovery 救援 |
