...
每个 `vendor + model/device + SoC + OTA fingerprint + compatibility_id + adapter` 组合独立验收。
进程启动或协议握手不等于画面替换成功。

最低测试：

1. 安装、重启、升级、卸载、Root 安全模式救援；SELinux 始终 Enforcing；
2. front/back/both 各启动、停止、切换 100 次；
3. 自动、720p30、1080p30，本地 MP4 与 MJPEG，0/90/180/270°、镜像、800 ms 断流失效；
...
9. 三次 adapter 失败进入 BootGuard，确认 OEM 相机仍可用；
10. Google 与 Xiaomi 至少分别覆盖目标 API/OTA 组合后再发布相应 Profile。
...
14. 删除或篡改 Profile/signature/adapter 任一文件，均必须 fail closed。

报告必须记录 compatibility ID、所有构件 SHA-256、系统/vendor fingerprint、Camera transport、
ROOT 授权结果、实际内容截图/哈希和失败日志。未完成的组合只能标 `UNVERIFIED`。
